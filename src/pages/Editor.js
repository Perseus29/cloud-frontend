...
const Editor = () => {
    ...
    useEffect(() => {
        if (valid) {
            const socketServer = io('https://cloud-backend-fhhw.onrender.com/');
    ...
    useEffect(() => {
        ...
        socket && socket.once('load-document', async document => {
            quill && await quill.setContents(document);
            quill && await quill.enable();
        });
